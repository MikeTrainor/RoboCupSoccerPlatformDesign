...
startTime = 0
endTime = 0 # both of these as globals
...
dirR=0
dirL=0
umax=450            #Max input for position control
u2max= 210
kp1= 1.65 #was 0.65
kp2=0.5 #was 1.2
flag =  0
    ...
    for rob in roboList:
        if (rob.ID != '-no ID!-') & (isinstance(roboList, type(None)) == 0):
            ...
            if (error2>=-10 and error2<=10):
                ...
                dirL=1 #May need to change depending on connection
            ...
            if (abs(error1) < 50 and abs(error2) <10): 
                kick= 0x01
            else:
                kick = 0
            if (error1<50):
