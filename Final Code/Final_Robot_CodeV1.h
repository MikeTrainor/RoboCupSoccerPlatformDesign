...
float Kp = 0.8, Kd = 1, Ki = 1.1; //Current Best is Kp = 5, Kd = 5, Ki = 1.05, Prev Best is Kp = 5, Kd = 5, Ki = 1.1, this is for step input
float u = 0, u2 = 0;
float r = 50, r2 = 50;
...
//  only handles 4 byte transfers in the ATMega code.
    ...
    while (1) {
        ...
        if ( my_nrf24l01p.readable() ) {
            ...
            pc.printf("%f",mes_vel);
            pc.putc(',');
            pc.printf("%f",des_vel2);
            pc.putc(',');
            pc.printf("%f",mes_vel2);
            pc.putc(',');
            pc.printf("%f",e);
            pc.putc(',');
            pc.printf("%f",e2);
