...
from PyQt5.QtWidgets import QSizePolicy, QSlider, QRadioButton, QGridLayout
...
def Kalman_Filter_pos():
		

	#Mathematical model of mass displacement
	#this kalman filter regulate on a x axis
	#This Kalman Filter filters out measurement noise and gives best estimate of where the robot is. 
	u=diff/dt#u is acceleration in this case. and it is supposed to be from the control in the controller
	Fx = numpy.matrix([[1,dt], [0,1]])#A Matrix (state transfer function)(x direction) 
	Bx=numpy.matrix([[dt**2/2], [dt]]) #B Matrix (control function)(x direction)
	Xx=numpy.matrix([[0],[0]])#initial state(belief values) (doesn't matter what it is.)(x direction)
	
	Fy = numpy.matrix([[1,dt], [0,1]])#A Matrix (state transfer function)(y direction) 
	By=numpy.matrix([[dt**2/2], [dt]]) #B Matrix (control function)(y direction)
	Xy=numpy.matrix([[0],[0]])#initial state(belief values) (doesn't matter what it is.)(y direction)
	mu, sigma = 0, 5
	noise=random.gauss(mu, sigma)#noise of pos from measurement (should be 1*1) assuming gaussian white noise with mean = 0 and variance of 5(fairly certain on the measurement data)

	#noise = randn(1)
	Px=numpy.matrix([[1,0],[0,1]])#state covariance matrix (doesn't matter what it is at the start as it will be iterated.)
	Qx=numpy.matrix([[0.01,0],[0 ,0.01]])#state predicted noise (when state transformation happens q(k) is process noise and covariance matrix is Q)
	Hx=numpy.matrix([[1,0]])#observation matrix (only pos is observable, vel is not)
	Rx=1#observation noise covariance 
	
	Py=numpy.matrix([[1,0],[0,1]])#state covariance matrix (doesn't matter what it is at the start as it will be iterated.)
	Qy=numpy.matrix([[0.01,0],[0 ,0.01]])#state predicted noise (when state transformation happens q(k) is process noise and covariance matrix is Q)
	Hy=numpy.matrix([[1,0]])#observation matrix (only pos is observable, vel is not)
	Ry=1#observation noise covariance 
	
	#x pos kalman filter
	for rob in roboList:
		Zx=rob.pos[0]+noise#i'm not sure if this is right should be directly from the measurement
		X_x=numpy.dot(Fx,Xx)+numpy.dot(Bx,ux)#predicted
		P_x=numpy.add(multi_dot([Fx,Px,Fx.transpose()]),Qx)#update of the covariance matrix for predicted value
		Kx=numpy.dot(P_x,Hx.transpose())/(multi_dot([Hx,P_x,Hx.transpose()])+Rx);# calculating Kalman gain
		Xx=numpy.add(X_x,Kx.dot(numpy.subtract(Zx,Hx.dot(X_x))))#belief which is a combination of estimation and measurement
		Px=multi_dot([numpy.subtract(numpy.identity(2),Kx.dot(Hx)), P_x])#update the covariance matrix
	#y pos kalman filter
	for rob in roboList:
		Zy=rob.pos[1]+noisey#i'm not sure if this is right should be directly from the measurement
		X_y=numpy.dot(Fy,Xy)+numpy.dot(By,uy)#predicted
		P_y=numpy.add(multi_dot([Fy,Py,Fy.transpose()]),Qy)#update of the covariance matrix for predicted value
		Ky=numpy.dot(P_y,Hy.transpose())/(multi_dot([Hy,P_y,Hy.transpose()])+Ry);# calculating Kalman gain
		Xy=numpy.add(X_y,Ky.dot(numpy.subtract(Zy,Hy.dot(X_y))))#belief which is a combination of estimation and measurement
		Py=multi_dot([numpy.subtract(numpy.identity(2),Ky.dot(Hy)), P_y])#update the covariance matrix
def Kalman_Filter_ball():
...
def mainLoop():
    ...
    global error1,error_prior1,error2,error_prior2,dt,derivative1,L,R,dirR,dirL,umax,u2max,kp
    global flag,kd,VrMax,VlMax,temp1,temp2,test,counter
    ...
    for rob in roboList:
        if(rob.ID != '-no ID!-') & (isinstance(roboList, type(None)) == 0):
            ...
            if(rob.radius < (rob.pos - len(img)) or rob.radius < (rob.pos - len(img[0]))):
                stoprobot(robotsID)
                continue
            else:
                ...
                print(samples_new[round(len(samples_new)/10)][0]) #X position with resolution of 10, if this is not good enough divide into smaller pieces perhaps
                print(samples_new[round(len(samples_new)/10)][1]) #X position with resolution of 10, if this is not good enough divide into smaller pieces perhaps
                ...
                error2=math.degrees((math.atan2(next_point_y-rob.pos[1],next_point_x-rob.pos[0])))-rob.angle #error2 for Mike's Added Value
				...
				# formula to make sure the minimum value of VrHex & VlHex =0x15 
